...
{
  # there are more than one sequences -> generate graph
...
# ---- read database file
# read IGMT ".tab" datafile based on the given input argument
...
seqdb$sequence_id = paste0( "ID_", 1:nrow(seqdb) )

# convert BASELINe parameters to "character" in order to avoid modifications
seqdb$baseline_sigma     = as.character(seqdb$baseline_sigma)
seqdb$baseline_ci_pvalue = as.character(seqdb$baseline_ci_pvalue)

# ---- group by clones
clones <- seqdb %>% group_by(clone_id) %>%
          do(CHANGEO=makeChangeoClone(., num_fields="duplicate_count",
                                         seq_fields=c("isotype",
                                                      "repertoire_id",
                                                      "baseline_sigma",
                                                      "baseline_ci_pvalue"),
